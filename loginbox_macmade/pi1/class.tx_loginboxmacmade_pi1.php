<?php
	...
	class tx_loginboxmacmade_pi1 extends tslib_pibase {
		...
		function main($content,$conf) {
			...
			if (($state == 'success' || $state == 'logout') && $this->GP['redirect_url']) {
			...
			} else if ($state == 'success' && $this->conf['redirect']) {
				...
				if ($this->piVars['forgot']) {
					...
					// Add header
					$htmlCode[] = $this->api->fe_makeStyledContent('h2','header',$this->conf[$state . '.']['header']);
					
					// Add message
					$htmlCode[] = $this->api->fe_makeStyledContent('div',$state,$this->conf[$state . '.']['message']);
					
					// Content
					$htmlCode[] = $this->api->fe_buildLoginBox($this->conf['pid'],$this->conf['loginBox.']['inputSize'],$this->conf['loginBox.']['method'],$this->conf['loginBox.']['target'],$this->conf['loginBox.']['wrap'],$this->conf['loginBox.']['layout'],'pi_loginbox_',$this->conf['permaLogin']);
					
					// Forgot password link
					if ($this->conf['forgotpassword'] && ($state == 'error' || $state == 'welcome' || $state == 'logout')) {
						
						// Link
						$forgotPwdLink = $this->pi_linkTP_keepPIvars($this->pi_getLL('forgotpassword.header'),array('forgot'=>'1'));
						
						// Add link
						$htmlCode[] = $this->api->fe_makeStyledContent('div','forgotpasswordLink',$forgotPwdLink);
					}
				}
		...
		function setDefaultLangValues() {
			...
			// Login states
			$states = array('welcome','success','error','status','logout');
		 ...
		 * of the loginbox.
		 * 
		 * @return		The section for the forgot password feature
		 */
		function forgotPassword() {
			...
			// Add header
			$htmlCode[] = $this->api->fe_makeStyledContent('h2','header',$this->pi_getLL('forgotpassword.header'));
			...
			if ($this->piVars['email']) {
				...
				if (t3lib_div::validEmail($this->piVars['email'])) {
					...
					// MySQL WHERE clause
					$whereClause = 'email=\'' . addslashes(trim($this->piVars['email'])) . '\' AND pid=' . intval($this->conf['pid']) . $this->cObj->enableFields('fe_users');
					
					// MySQL query
					$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*','fe_users',$whereClause);
					
					// Get FE user
					if ($user = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
						
						// Success message
						$message = '<strong>' . $this->pi_getLL('forgotpassword.success') . '</strong>';
						...
						$mailMsg = sprintf($this->pi_getLL('forgotpassword.mailmsg'),$username,t3lib_div::getIndpEnv('TYPO3_HOST_ONLY'),$user['username'],$user['password'],'http://' . t3lib_div::getIndpEnv('HTTP_HOST') . '/' . $this->api->fe_linkTP_unsetPIvars_url(array(),array('forgot','email','submit')));
						
						// Send mail
						$this->cObj->sendNotifyEmail($mailMsg,$username . ' <' . trim($this->piVars['email']) . '>','',$this->conf['mailer.']['from'],$this->conf['mailer.']['fromName'],$this->conf['mailer.']['replyTo']);
						
					} else {
						
						// No user
						$message = '<strong>' . $this->pi_getLL('forgotpassword.error.nouser') . '</strong>';
					}
					
				} else {
					
					// Message
					$message = '<strong>' . $this->pi_getLL('forgotpassword.error.unvalidemail') . '</strong>';
				}
			} else {
			...
			// Add message
			$htmlCode[] = $this->api->fe_makeStyledContent('div','forgotpassword',$message);
			...
			if ($success) {
				...
				// Start form
				$htmlCode[] = '<form action="' . htmlspecialchars(t3lib_div::getIndpEnv('REQUEST_URI')) . '" method="post" enctype="' . $GLOBALS['TYPO3_CONF_VARS']['SYS']['form_enctype'] . '">';
				
				// Start paragraph
				$htmlCode[] = $this->api->fe_makeStyledContent('p','forgotpasswordForm',false,1,0,1);
				
				// Input text
				$htmlCode[] = $this->pi_getLL('forgotpassword.email') . ':&nbsp;';
				
				// Input
				$htmlCode[] = '<input name="' . $this->prefixId . '[email]" type="text" size="20">&nbsp;';
				
				// Submit
				$htmlCode[] = '<input name="' . $this->prefixId . '[submit]" type="submit" value="' . $this->pi_getLL('forgotpassword.send') . '"></p>';
