<?php
	...
	***************************************************************/
	...
	class tx_weledababy_pi2 extends tslib_pibase {
		// Same as class name
		var $prefixId = 'tx_weledababy_pi2';
		
		// Path to this script relative to the extension dir.
		var $scriptRelPath = 'pi2/class.tx_weledababy_pi2.php';
		...
		function main($content,$conf) {
			...
			if (t3lib_div::_POST('logintype') || $GLOBALS['TSFE']->loginUser) {
				...
				if ($this->conf['redirect'] && $GLOBALS['TSFE']->loginUser) {
					...
					// Login box
					$content = $this->api->fe_makeStyledContent('div','login-title',$this->pi_getLL('connect')) . $this->api->fe_makeStyledContent('div','login-error',$this->pi_getLL('connect.error')) . $this->api->fe_makeStyledContent('div','lobinbox',$this->api->fe_buildLoginBox($this->conf['storagePid']));
					
				}
			} else if (empty($this->conf['agree_text']) || array_key_exists('agree',$this->piVars)) {
		...
		function buildForm() {
			...
			// Birth
			$templateMarkers['###BIRTH###'] = $this->writeInput('birth');
			...
			$content = $this->api->fe_makeStyledContent('form','inputForm',$this->api->fe_renderTemplate($templateMarkers,'###MAIN###'),1,0,0,array('action'=>$this->pi_linkTP_keepPIvars_url(array('submit'=>'1')),'enctype'=>$GLOBALS['TYPO3_CONF_VARS']['SYS']['form_enctype'],'method'=>'post'));
		...
		function writeInput($fieldName) {
			...
			switch($fieldName) {
				...
				case 'address':
					...
					// Textarea
					$input = '<textarea id="' . $id . '" name="' . $id . '" rows="' . $this->conf['textareaRows'] . '" cols="' . $this->conf['textareaCols'] . '">' . $value . '</textarea>';
				break;
				...
				case 'password':
					...
					// Password input
					$input = '<input type="password" id="' . $id . '[0]" name="' . $id . '[0]" size="' . $this->conf['inputSize'] . '" /><br /><input type="password" id="' . $id . '[1]" name="' . $id . '[1]" size="20" />';
				break;
				...
				default:
					...
					// Text input
					$input = '<input type="text" id="' . $id . '" name="' . $id . '" value="' . $value . '" size="' . $this->conf['inputSize'] . '" />';
				break;
			}
		...
		function insertUser() {
			...
			// Check for a birth date
			if (array_key_exists('birth',$this->fedata[$this->prefixId])) {
				
				// Add date
				$profile['birth'] = strtotime($this->fedata[$this->prefixId]['birth']);
			}
	 ...
	 * XCLASS inclusion
	 */
	if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/weleda_baby/pi1/class.tx_weledababy_pi2.php']) {
		include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/weleda_baby/pi1/class.tx_weledababy_pi2.php']);
	}
?>
