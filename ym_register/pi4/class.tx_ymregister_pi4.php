 ...
 */
class tx_ymregister_pi4 extends tx_oop_Plugin_Base
{
    /**
    ...
    protected function _getPluginContent( tx_oop_Xhtml_Tag $content )
    {
        ...
        if( $imageId = self::$_tsfe->fe_user->getKey( 'ses', 'imageUserId' ) ) {
            
            $imagesRes = self::$_t3Db->exec_SELECTquery(
                'uid',
                'tx_ymimage_xml',
                'usr_id=' . self::$_t3Db->fullQuoteStr( $imageId, 'tx_ymimage_xml' )
            );
            
            $image = self::$_t3Db->sql_fetch_assoc( $imagesRes );
            ...
            $fbLink[ 'href' ] = 'javascript:FacebookStreamPublishVote(' . $image[ 'uid' ] . ', ' . self::$_tsfe->fe_user->user[ 'uid' ] . ')';
            
            $fbLink->addTextData( $this->_lang->fbPublish );
            
        } elseif( $imageId = t3lib_div::_GET( 'imageUid' ) ) {
...
// X-Class inclusion
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ym_register/pi4/class.tx_ymregister_pi4.php'])	{
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ym_register/pi4/class.tx_ymregister_pi4.php']);
}
